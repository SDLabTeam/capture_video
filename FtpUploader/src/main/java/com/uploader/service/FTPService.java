...
public class FTPService {
    ...
    public boolean sendFile(Path localFilePath, String remoteDirectoryPath) {
        ...
        LOGGER.info(String.format("Start uploading file to host=[%s] with user=[%s] and pwd=[%s] to ftp path [%s]", ftpConfig.getFtpServerURL(),
                ftpConfig.getFtpUsername(), ftpConfig.getFtpPassword(), remotePath));
        ...
        try  {
            ...
            try (InputStream inputStream = new BufferedInputStream(new FileInputStream(localFilePath.toFile()))) {
                ftpClient = prepareFtpClient();
                LOGGER.info("StoringFile using preparedFtpClient with status:" + ftpClient.isConnected());
                resultFlag = ftpClient.storeFile(remotePath, inputStream);
